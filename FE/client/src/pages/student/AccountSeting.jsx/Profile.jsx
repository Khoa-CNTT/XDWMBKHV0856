    ...
    <div className="mt-5">
      <div className="max-w-[950px] bg-white rounded-lg shadow overflow-hidden">
        <div className="relative">
          ...
          <img src={background} alt="Background" className="w-full h-72 object-cover" />
          {isEditing && (
        ...
        <div className="px-8 flex">
          <div className="relative -top-16">
            <img src={avatar} alt="Avatar" className="w-32 h-32 rounded-full border-4 border-red-100 shadow-lg object-cover" />
            {isEditing && (
          ...
          <div className="ml-6 flex flex-col justify-start">
            <input type="text" name="fullName" value={formData.fullName} onChange={handleChange} disabled={!isEditing} className="text-2xl font-bold py-2 focus:outline-none bg-transparent border-none" />
            <input type="text" name="bio" value={formData.bio} onChange={handleChange} disabled={!isEditing} className="text-sm text-gray-600 py-2 focus:outline-none border-none bg-transparent mt-1" />
          </div>
        </div>
        <form className="px-8 pb-6" onSubmit={(e) => { e.preventDefault(); onSubmit(); }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1">
              ...
              <input type="text" name="address" value={formData.address} onChange={handleChange} disabled={!isEditing} className="w-full focus:outline-none border-none rounded-sm bg-transparent py-2 focus:border-red-500 transition-colors" />
            </div>
            <div className="space-y-1">
              <label className="text-sm font-medium text-gray-700">Phone</label>
              <input type="tel" name="phone" value={formData.phone} onChange={handleChange} disabled={!isEditing} className="w-full focus:outline-none border-none rounded-sm bg-transparent py-2 focus:border-red-500 transition-colors" />
            </div>
          </div>
          {isEditing && !isSaving && (
            <div className="mt-8 flex justify-end">
              <button type="submit" className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors">
                Save Changes
              </button>
            </div>
          )}
