...
import {
    getCourse, updateCourse, updateChapter, deleteChapter, deleteLecture,
    updateFileLecture, updateLecture, createChapter, createLecture, updateImageCourse
    ...
    useEffect(() => {
        const fetchCourseData = async () => {
            ...
            try {
                ...
                setRelatedSkill(data.fields.reduce((acc, field) => {
                    acc[field.id] = field.skills ? field.skills.map(skill => skill.name) : [];
                    return acc;
                }, {}));
        ...
        try {
            ...
            for (const section of sectionsToUpdate) {
                ...
                for (const lesson of section.lessons || []) {
                    const originalLesson = originalSection?.lessons?.find(l => l.id === lesson.id);
        ...
        <div className="fixed inset-0 bg-red-maroon bg-blend-overlay bg-cover animate-floating-books flex items-center justify-center z-50">
            ...
            <div className="bg-red-50 from-white via-red-50 to-purple-50 w-[90vw] max-w-5xl h-[90vh] rounded-2xl shadow-2xl p-8 relative overflow-y-auto">
                    ...
                    <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <label className="w-40 h-40 border-2 border-dashed rounded-lg cursor-pointer flex items-center justify-center bg-white overflow-hidden relative">
                            {courseImage ? (
                                ...
                                <img
                                    src={`${import.meta.env.VITE_COURSE_IMAGE_URL}/${courseId}/${image}`} // Ảnh từ server
                                    alt={title}
                                    className="w-full h-full object-cover"
                                />
                            ) : (
                ...
                <div className="mt-6">
                    ...
                    <textarea
                        className={`w-full p-3 border ${errors.shortIntroduce ? "border-red-500" : "border-black"} rounded-lg bg-white`}
                        ...
                        required
                    />
                    {errors.shortIntroduce && <p className="text-red-500 text-sm">SShort Introduce is required.</p>}
                </div>
                ...
                <div className="mt-6 border border-black rounded-xl p-4 bg-white">
                    <div className="flex gap-4 mb-4 p-2 justify-between">
                        <label className="font-semibold block text-black-600">
                            Field
                        </label>
                        <input
                            type="text"
                        ...
                        {Object.entries(skillsByField).map(([fieldId, skillList]) => (
                            <div key={fieldId}>
                                <h4 className="font-semibold text-red-600 mb-2">
                                    {fields.find((f) => f.id === Number(fieldId))?.name || "Unknown Field"}
                                </h4>
                                <div className="flex flex-wrap gap-2">
                                    {skillList
                                        .filter((skill) =>
                                        ...
                                        .map((skill) => (
                                            <button
                                                key={skill.id}
                                                onClick={() => handleRelatedSkillChange(fieldId, skill.name)}
